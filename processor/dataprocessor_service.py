...
class DataProcessorService:
    ...
    def save_to_database(self, result: DataFrame) -> None:
        ...
        if result is not None:
            try:
                db_connector = SQLStoreConnectorFactory().get_connector(
                    self.db_connection_url)  # инициализируем соединение
                db_connector.start_transaction()  # начинаем выполнение запросов (открываем транзакцию)

                if self.datasource.startswith('doctor'):
                    insert_rows_into_processed_data_doctor(db_connector, result)

                if self.datasource.startswith('patient'):
                    insert_rows_into_processed_data_patient(db_connector, result)

                if self.datasource.startswith('appointment'):
                    insert_rows_into_processed_data_appointment(db_connector, result)
