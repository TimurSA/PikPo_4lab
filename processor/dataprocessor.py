...
class DataProcessor(ABC):
    ...
    def run(self) -> None:
        ...
        cleaned_result = None
        if "experience" in self._dataset.columns:
            cleaned_result = self.remove_col_by_name(self._dataset, ["experience"])
            # cleaned_result = self.get_query(cleaned_result, "specialization == 'allergist'")
            self.result = cleaned_result
            return
        elif "contact_info" in self._dataset.columns:
            self.result = self.remove_col_by_name(self._dataset, ["contact_info"])
            return
        elif 'appointment_id' in self._dataset.columns:
            self.result = self.remove_col_by_name(self._dataset, ["appointment_id"])
            return
